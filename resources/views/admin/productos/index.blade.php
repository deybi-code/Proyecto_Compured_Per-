...
<table class="table-responsive-card" width="100%" cellpadding="15" style="border-collapse:collapse;">
    ...
    @foreach($productos as $p)
        ...
        <tr style="border-bottom:1px solid var(--border); transition: background 0.2s;" class="product-row">
            ...
            {{-- ACCIONES --}}
            <td data-label="Acciones" style="display:flex;gap:8px;flex-wrap:wrap; align-items: center;">

                {{-- EDITAR --}}
                <a href="{{ route('admin.productos.edit', $p) }}"
                   style="background:#0056b3;color:white;padding:8px 14px;border-radius:8px;text-decoration:none; font-weight: 600; font-size: 13px; display: inline-flex; align-items: center; gap: 6px; transition: all 0.2s;"
                ...
                <form method="POST" action="{{ route('admin.productos.destroy', $p) }}" style="display:inline;">
                    ...
                    <button onclick="return confirm('¿Eliminar este producto?')"
                            style="background:#ef4444;color:white;padding:8px 12px;border:none;border-radius:8px; cursor: pointer; font-weight: 600; font-size: 13px; display: inline-flex; align-items: center; gap: 6px; transition: all 0.2s;"
